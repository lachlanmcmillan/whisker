...
import { readAllFeeds, readFeedById, upsertFeed, deleteFeed, updateEntryOpenedAt, } from "./models/feed.model";
...
const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        if (method === "POST" && url.pathname === "/feeds") {
            ...
            if (!body.url)
                return json({ error: { code: "fetch_failed", message: "url is required" } }, 400);
            const fetchResult = await fetchFeed(body.url);
            if (fetchResult.error)
                return json(fetchResult, 400);
            const upsertResult = upsertFeed(fetchResult.data);
            if (upsertResult.error)
                return json(upsertResult, 500);
        ...
        if (method === "POST" && url.pathname.match(/^\/feeds\/\d+\/refresh$/)) {
            ...
            if (feedResult.error)
                return json(feedResult, 500);
            if (!feedResult.data)
                return json({ error: { code: "feed_not_found", message: "Feed not found" } }, 404);
            const feedUrl = feedResult.data.feedUrl || feedResult.data.link;
            const fetchResult = await fetchFeed(feedUrl);
            if (fetchResult.error)
                return json(fetchResult, 500);
            const upsertResult = upsertFeed(fetchResult.data);
            if (upsertResult.error)
                return json(upsertResult, 500);
